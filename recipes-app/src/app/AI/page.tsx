...
import { IconArrowUp, IconArrowDown, IconX, IconShoppingCart, IconListCheck, IconChefHat, IconScale, IconCalculator, IconPlus, IconTrash, IconApple } from '@tabler/icons-react';
...
const ImageSlider: React.FC = () => {
  ...
  const isValidImageUrl = async (url: string): Promise<boolean> => {
    try {
      ...
      const isKnownImageDomain = knownImageDomains.some(domain => url.includes(domain));
...
export default function AiRecipePage() {
  ...
  const [activeMode, setActiveMode] = useState<'recipe-generator' | 'calorie-counter'>('recipe-generator');
  const [mealEntries, setMealEntries] = useState<Array<{
    id: string;
    description: string;
    isAI: boolean;
    portion: number;
  ...
  const [isRecipeDropdownOpen, setIsRecipeDropdownOpen] = useState<boolean>(false);
  ...
  const [showNutrientDetails, setShowNutrientDetails] = useState<boolean>(false);
    ...
    const calculateTotals = () => {
      ...
      mealEntries.forEach((entry) => {
        if (entry.nutritionalInfo) {
          ...
          totals.calories = safeAdd(totals.calories, entry.nutritionalInfo.calories * scalingFactor);
          totals.protein = safeAdd(totals.protein, entry.nutritionalInfo.protein * scalingFactor);
          totals.fat = safeAdd(totals.fat, entry.nutritionalInfo.fat * scalingFactor);
          totals.carbohydrates = safeAdd(totals.carbohydrates, entry.nutritionalInfo.carbohydrates * scalingFactor);
          totals.fiber = safeAdd(totals.fiber, entry.nutritionalInfo.fiber * scalingFactor);
          totals.sugar = safeAdd(totals.sugar, entry.nutritionalInfo.sugar * scalingFactor);
          totals.sodium = safeAdd(totals.sodium, entry.nutritionalInfo.sodium * scalingFactor);
          totals.cholesterol = safeAdd(totals.cholesterol, entry.nutritionalInfo.cholesterol * scalingFactor);
        }
      });
  ...
  const handleAddRecipeEntry = async (recipeId: number) => {
    ...
    try {
      ...
      if (recipe.recipe_ingredients && Array.isArray(recipe.recipe_ingredients)) {
        ...
        ingredientBreakdown = recipe.recipe_ingredients.map((ingredient: any) => {
          const caloriesPerIngredient = totalCalories / recipe.recipe_ingredients.length; // Simple estimation
          return {
            name: ingredient.ingredient?.name || 'Unknown ingredient',
            quantity: ingredient.quantity,
            unit: ingredient.unit,
            displayText: `${ingredient.ingredient?.name || 'Unknown'} (${ingredient.quantity}${ingredient.unit})`,
            ...
            percentOfTotal: ((caloriesPerIngredient / totalCalories) * 100).toFixed(1)
          };
        });
      }
      
      console.log('Recipe portion:', recipe.portion, 'User portion:', newMealPortion);
      ...
      const newEntry = {
        ...
        nutritionalInfo: recipe.nutritional_info ? {
          ...
          calories: (recipe.nutritional_info.calories / recipeDefaultPortion) * newMealPortion,
          protein: (recipe.nutritional_info.protein / recipeDefaultPortion) * newMealPortion,
          fat: (recipe.nutritional_info.fat / recipeDefaultPortion) * newMealPortion,
          carbohydrates: (recipe.nutritional_info.carbohydrates / recipeDefaultPortion) * newMealPortion,
          fiber: (recipe.nutritional_info.fiber / recipeDefaultPortion) * newMealPortion,
          sugar: (recipe.nutritional_info.sugar / recipeDefaultPortion) * newMealPortion,
          sodium: (recipe.nutritional_info.sodium / recipeDefaultPortion) * newMealPortion,
          cholesterol: (recipe.nutritional_info.cholesterol / recipeDefaultPortion) * newMealPortion,
  ...
  return (
    <div className="container mx-auto px-2 sm:px-4 py-4 sm:py-8">
      ...
      {(loading || loadingNutrition) && (
        <div className="fixed inset-0 flex justify-center items-center bg-base-200/70 z-10">
          <button className="btn btn-square btn-lg loading">Loading</button>
        </div>
      )}

      <div className="flex flex-col items-center mb-4 sm:mb-8">
        <IconChefHat size={36} className="text-primary mb-2 sm:mb-4 sm:text-5xl" />
        <h1 className="text-3xl sm:text-4xl font-bold mb-2 sm:mb-4 text-center">
          {activeMode === 'recipe-generator' ? 'AI Recipe Generator' : 'Calorie Counter'}
        </h1>
      ...
      <div className="flex flex-wrap justify-center gap-2 sm:gap-4 mb-6 sm:mb-8">
        <button
          className={`btn btn-sm sm:btn-md ${activeMode === 'recipe-generator' ? 'btn-primary' : 'btn-outline'}`}
          onClick={() => setActiveMode('recipe-generator')}
        >
          <IconChefHat className="mr-1 sm:mr-2" size={16} />
          <span className="text-xs sm:text-sm">Recipe Generator</span>
        </button>
        
        <button
          className={`btn btn-sm sm:btn-md ${activeMode === 'calorie-counter' ? 'btn-primary' : 'btn-outline'}`}
          onClick={() => setActiveMode('calorie-counter')}
        >
          <IconCalculator className="mr-1 sm:mr-2" size={16} />
          <span className="text-xs sm:text-sm">Count Calories</span>
        </button>
        ...
        >
          <span className="text-xs sm:text-sm">Browse AI Recipes</span>
        </button>
      </div>
      ...
      {activeMode === 'recipe-generator' && (
        <>
          {/* Input Card */}
          <div className="card bg-base-100 shadow-xl rounded-xl overflow-hidden mb-8">
            <div className="p-6 sm:p-8">
              <h2 className="text-2xl font-bold mb-6">Create Your Recipe</h2>

      <div className="mb-4">
                <label className="label font-medium">Ingredients (comma separated):</label>
        <input
          type="text"
        ...
        />
              ...
              <div className="mb-6">
                <label className="label font-bold">Any dietary or flavor preferences?</label>
        <input
          type="text"
      ...
      {generatedRecipe && (
          ...
          />
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-6 text-white">
                  <h2 className="text-2xl sm:text-3xl font-bold mb-2 drop-shadow-lg">{generatedRecipe.title}</h2>
            ...
            {generatedRecipe.category && (
                ...
                {generatedRecipe.category.charAt(0).toUpperCase() + generatedRecipe.category.slice(1)}
                    ...
                    <span className="text-sm opacity-90">
                      {currentPortions} {currentPortions === 1 ? 'serving' : 'servings'}
                    </span>
                  </div>
                </div>
          </div>
              ...
              <div className="p-4 sm:p-6 md:p-8">
                ...
                {/* Description Section */}
                <p className="text-base sm:text-lg mb-6 sm:mb-8">{generatedRecipe.description}</p>
                ...
                {scaledNutritionalInfo && (
                  <div className="grid grid-cols-2 gap-4 mb-6 sm:mb-8 p-4 bg-base-200 rounded-xl">
                    <div className="text-center">
                      <div className="stat-value text-primary text-xl sm:text-2xl">{scaledNutritionalInfo.calories.toFixed(2)}</div>
                      <div className="stat-title text-xs sm:text-sm">Calories</div>
          </div>
                    <div className="text-center">
                      <div className="stat-value text-primary text-xl sm:text-2xl">{scaledNutritionalInfo.protein.toFixed(2)}g</div>
            ...
            {generatedRecipe.ingredients.map((ing: any, idx: number) => (
                      <div key={idx} className="flex items-center gap-2 p-2 rounded-lg hover:bg-base-200 transition-colors">
                        <input
                          type="checkbox"
                          className="checkbox checkbox-primary checkbox-sm"
                        />
                        <span className="text-sm sm:text-base">
                          <span className="font-medium">{(ing.quantity * scalingFactor).toFixed(2)} {ing.unit}</span> {ing.name}
                        </span>
                      </div>
                    ))}
                  </div>
            ...
            {generatedRecipe.steps.map((step: any) => (
                      <div key={step.id || step.order} className="p-3 sm:p-4 bg-base-200 rounded-xl">
          ...
          {scaledNutritionalInfo && (
            <>
              <div ref={fullNutritionalInfoRef}></div>
                    <h3 className="text-xl sm:text-2xl font-semibold mt-8 mb-4">Full Nutritional Information</h3>
          ...
          {generatedRecipe.per_ingredient_nutritional_info && generatedRecipe.per_ingredient_nutritional_info.length > 0 && (
            <>
                    <h3 className="text-xl sm:text-2xl font-semibold mt-8 mb-4">Per-Ingredient Nutritional Information</h3>
                    <div className="overflow-x-auto">
                <table className="table table-zebra w-full">
                  <thead>
                    <tr>
                    ...
                    {generatedRecipe.per_ingredient_nutritional_info.map((info: any, idx: number) => (
                      <tr key={idx}>
                        ...
                        <td>{(info.calories * scalingFactor).toFixed(2)}</td>
                              <td>{(info.protein * scalingFactor).toFixed(2)}g</td>
                              <td>{(info.fat * scalingFactor).toFixed(2)}g</td>
                              <td>{(info.carbohydrates * scalingFactor).toFixed(2)}g</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      {/* Calorie Counter Mode Content */}
      {activeMode === 'calorie-counter' && (
        <div className="calorie-counter-mode">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-6">
            {/* Left Column - Input Area */}
            <div className="space-y-3 sm:space-y-4">
              <div className="card bg-base-100 shadow-md border border-base-200 overflow-hidden">
                <div className="card-body p-3 sm:p-6">
                  <h3 className="card-title text-md sm:text-lg font-bold mb-1 sm:mb-2">Add New Food Entry</h3>
                    ...
                    <div className="grid grid-cols-2 gap-x-3 gap-y-2 sm:gap-4 mb-3 sm:mb-4">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => setNewMealPortion(Number(e.target.value))}
                    ...
                    <div className="dropdown-content z-50 menu shadow bg-base-100 rounded-box w-full max-h-60 sm:max-h-80 overflow-auto mt-1 border border-base-300">
                      <ul className="p-2 sm:p-3">
                        {filteredRecipes.map(recipe => (
                          <li key={recipe.id} className="border-b last:border-b-0">
                            <button
                              onClick={() => {
                            ...
                            >
                              ...
                              <div>
                                <div className="font-medium text-sm sm:text-base">{recipe.title}</div>
                                {recipe.category && (
              ...
              <div className="card bg-base-100 shadow-md border border-base-200">
                <div className="card-body p-3 sm:p-6">
                  <h3 className="card-title text-md sm:text-lg font-bold mb-3">Daily Nutrition Summary</h3>
                  ...
                  <div className="stats stats-vertical lg:stats-horizontal shadow bg-base-200 w-full mb-4">
                    <div className="stat">
                      ...
                      <div className="stat-value text-primary">{Math.round(dailyTotals.calories)}</div>
                      <div className="stat-desc">{Math.round((dailyTotals.calories / 2000) * 100)}% daily value</div>
                    </div>
                    
                    <div className="stat">
                      <div className="stat-title">Protein</div>
                      <div className="stat-value text-secondary">{Math.round(dailyTotals.protein)}g</div>
                      <div className="stat-desc">{Math.round((dailyTotals.protein / 50) * 100)}% daily value</div>
                    </div>
                    
                    <div className="stat">
                      <div className="stat-title">Carbs</div>
                      <div className="stat-value text-accent">{Math.round(dailyTotals.carbohydrates)}g</div>
                      <div className="stat-desc">{Math.round((dailyTotals.carbohydrates / 300) * 100)}% daily value</div>
                    </div>
                    
                    <div className="stat">
                      <div className="stat-title">Fat</div>
                      <div className="stat-value">{Math.round(dailyTotals.fat)}g</div>
                      <div className="stat-desc">{Math.round((dailyTotals.fat / 65) * 100)}% daily value</div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    <div className="bg-base-200 p-3 rounded-lg">
                      ...
                      <div className="text-lg font-bold">{Math.round(dailyTotals.fiber)}g</div>
                      <div className="text-xs opacity-70">{Math.round((dailyTotals.fiber / 25) * 100)}% daily value</div>
                    </div>
                    
                    <div className="bg-base-200 p-3 rounded-lg">
                      <div className="font-semibold text-sm">Sugar</div>
                      <div className="text-lg font-bold">{Math.round(dailyTotals.sugar)}g</div>
                      <div className="text-xs opacity-70">{Math.round((dailyTotals.sugar / 50) * 100)}% daily value</div>
                    </div>
                    
                    <div className="bg-base-200 p-3 rounded-lg">
                      <div className="font-semibold text-sm">Sodium</div>
                      <div className="text-lg font-bold">{Math.round(dailyTotals.sodium)}mg</div>
                      <div className="text-xs opacity-70">{Math.round((dailyTotals.sodium / 2300) * 100)}% daily value</div>
                    </div>
                    
                    <div className="bg-base-200 p-3 rounded-lg">
                      <div className="font-semibold text-sm">Cholesterol</div>
                      <div className="text-lg font-bold">{Math.round(dailyTotals.cholesterol)}mg</div>
                      <div className="text-xs opacity-70">{Math.round((dailyTotals.cholesterol / 300) * 100)}% daily value</div>
                    </div>
                  </div>
            ...
            <div className="space-y-3 sm:space-y-4">
              <div className="card bg-base-100 shadow-md border border-base-200">
                <div className="card-body p-3 sm:p-6">
                  <h3 className="card-title text-md sm:text-lg font-bold mb-1 sm:mb-2">Food Entries</h3>
                  
                  {mealEntries.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-6 text-gray-500">
                      <IconApple size={36} className="mb-2" />
                      <p className="text-center text-sm">No food entries yet. Add what you've eaten using the form.</p>
                    </div>
                  ) : (
                    <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-1">
                      {/* Group entries by meal time */}
                      {['breakfast', 'lunch', 'dinner', 'snack'].map(mealTime => {
                        const mealTimeEntries = mealEntries.filter(entry => entry.time === mealTime);
                        if (mealTimeEntries.length === 0) return null;
                        
                        return (
                          <div key={mealTime} className="mb-3">
                            <h4 className="font-bold text-sm sm:text-base capitalize mb-2 sticky top-0 bg-base-100 py-1 z-10">{mealTime}</h4>
                              ...
                              >
                                {entry.error ? (
                                  <div className="flex justify-between items-center">
                                    <div className="flex-1">
                                      <div className="font-semibold text-md">{entry.description}</div>
                                      <div className="text-sm text-error">{entry.error}</div>
                                    </div>
                                    <button
                                      onClick={() => handleRemoveMealEntry(entry.id)}
                                  ...
                                  <div className="flex justify-between items-center">
                                    <div className="flex-1">
                                      ...
                                      <div className="flex items-center mt-2">
                                        ...
                                        <span className="text-sm opacity-70">Fetching nutritional information...</span>
                                      </div>
                                    </div>
                                    <button
                                      onClick={() => handleRemoveMealEntry(entry.id)}
                                  ...
                                  <div className="flex justify-between items-start">
                                    <div className="flex-1">
                                      ...
                                      <div className="text-xs opacity-70">
                                        {entry.fromRecipe
                                          ? `Recipe entry • ${entry.portion} portion${entry.portion !== 1 ? 's' : ''}`
                                          : `Custom entry • ${entry.portion} portion${entry.portion !== 1 ? 's' : ''}`}
                                        {entry.quantity && entry.unit && ` (${entry.quantity} ${entry.unit})`}
                                      </div>
                                      {entry.source && (
                                        <div className="text-xs flex items-center mt-1">
                                          <span className="opacity-70">Data source:</span>
                                          <span className="ml-1 badge badge-xs badge-outline">{entry.source}</span>
                                        </div>
                                      )}
                                      {entry.nutritionalInfo ? (
                                        <div className="mt-2">
                                          <div className="flex flex-col gap-1">
                                            <div className="flex justify-between items-center">
                                              ...
                                              <span>{Math.round(entry.nutritionalInfo.calories * entry.portion)}</span>
                                            </div>
                                            <div className="flex justify-between text-sm">
                                              <span>Protein:</span>
                                              <span>{(entry.nutritionalInfo.protein * entry.portion).toFixed(1)}g</span>
                                            </div>
                                            <div className="flex justify-between text-sm">
                                              <span>Carbs:</span>
                                              <span>
                                                {(entry.nutritionalInfo.carbohydrates * entry.portion).toFixed(1)}g
                                              </span>
                                            </div>
                                            <div className="flex justify-between text-sm">
                                              <span>Fat:</span>
                                              <span>{(entry.nutritionalInfo.fat * entry.portion).toFixed(1)}g</span>
                                            </div>
                                                ...
                                                <ul className="text-xs space-y-1">
                                                  {entry.components.map((component, idx) => (
                                                    <li key={idx} className="flex justify-between">
                                                      ...
                                                      <span>{Math.round(component.nutritionalInfo.calories)} cal</span>
                                                    </li>
                                                  ))}
                                                </ul>
                                              </div>
                                            ...
                                            {entry.nutritionalInfo.ingredients && entry.nutritionalInfo.ingredients.length > 0 && (
                                              <div className="mt-1 bg-base-300 p-2 rounded-md">
                                                <div className="text-xs font-semibold mb-1">Ingredients:</div>
                                                <ul className="text-xs space-y-1">
                                                  {entry.nutritionalInfo.ingredients.map((ingredient: any, idx: number) => (
                                                    <li key={idx} className="flex justify-between">
                                                      <span>{ingredient.displayText || `${ingredient.name} (${ingredient.quantity}${ingredient.unit})`}</span>
                                                      {ingredient.calories && <span>{Math.round(ingredient.calories * entry.portion)} cal</span>}
                                                    </li>
                                                  ))}
                                                </ul>
                                              </div>
                                        ...
                                        <div className="mt-2 text-sm opacity-70">No nutritional data available</div>
                                      )}
                                      
                                      <div className="mt-3">
                                        <div className="flex items-center">
                                          <span className="text-xs mr-2">Adjust portion:</span>
                                          <input
                                            type="number"
                                            min="0.25"
                                            max="10"
                                            step="0.25"
                                            value={entry.portion}
                                            onChange={(e) => handleUpdatePortion(entry.id, parseFloat(e.target.value))}
      ...
      {/* Error display - use for both features */}
      {error && (
        <div className="alert alert-error mb-4 sm:mb-8 text-xs sm:text-sm">
          <p>{error}</p>
        </div>
      )}
      ...
      {scroll.y > 100 && (
        <button
          ...
          className="btn btn-circle btn-sm sm:btn-md fixed bottom-4 sm:bottom-6 right-4 sm:right-6 transition-transform hover:scale-110"
      ...
      {showNutrientDetails && (
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <div className="fixed inset-0 transition-opacity" aria-hidden="true" onClick={() => setShowNutrientDetails(false)}>
              ...
              <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div className="sm:flex sm:items-start">
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                    <h3 className="text-lg leading-6 font-medium text-gray-900 mb-4">Daily Nutrition Details</h3>
                      ...
                      <table className="table table-zebra w-full">
                        ...
                        <tbody>
                          <tr>
                            ...
                            <td>{Math.round((dailyTotals.calories / 2000) * 100)}%</td>
                          </tr>
                          <tr>
                            <td>Protein</td>
                            <td>{Math.round(dailyTotals.protein)}g</td>
                            <td>{Math.round((dailyTotals.protein / 50) * 100)}%</td>
                          </tr>
                          <tr>
                            <td>Fat</td>
                            <td>{Math.round(dailyTotals.fat)}g</td>
                            <td>{Math.round((dailyTotals.fat / 65) * 100)}%</td>
                          </tr>
                          <tr>
                            <td>Carbohydrates</td>
                            <td>{Math.round(dailyTotals.carbohydrates)}g</td>
                            <td>{Math.round((dailyTotals.carbohydrates / 300) * 100)}%</td>
                          </tr>
                          <tr>
                            <td>Fiber</td>
                            <td>{Math.round(dailyTotals.fiber)}g</td>
                            <td>{Math.round((dailyTotals.fiber / 25) * 100)}%</td>
                          </tr>
                          <tr>
                            <td>Sugar</td>
                            <td>{Math.round(dailyTotals.sugar)}g</td>
                            <td>{Math.round((dailyTotals.sugar / 50) * 100)}%</td>
                          </tr>
                          <tr>
                            <td>Sodium</td>
                            <td>{Math.round(dailyTotals.sodium)}mg</td>
                            <td>{Math.round((dailyTotals.sodium / 2300) * 100)}%</td>
                          </tr>
                          <tr>
                            <td>Cholesterol</td>
                            <td>{Math.round(dailyTotals.cholesterol)}mg</td>
                            <td>{Math.round((dailyTotals.cholesterol / 300) * 100)}%</td>
                          </tr>
                        </tbody>
                      </table>
