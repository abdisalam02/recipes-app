...
import { IconChefHat, IconClock, IconArrowDown, IconHeart, IconHeartFilled, IconShoppingCart, IconListCheck } from '@tabler/icons-react';
...
const StepsModal = ({ steps, onClose }: { steps: { order: number; description: string }[]; onClose: () => void }) => {
...
export default function DailyRecipeDetailPage() {
  ...
  const [availableIngredients, setAvailableIngredients] = useState<{ [key: string]: boolean }>({});
  ...
  const parseSteps = (instructions: string): Array<{ order: number; description: string }> => {
  ...
  useEffect(() => {
    const fetchRecipeDetails = async () => {
      try {
        ...
        const matchingRecipe = recipes.find((r: DailyRecipeItem) => r.id.toString() === recipeId);
        ...
        setCurrentPortions(matchingRecipe.servings || matchingRecipe.portion || 1);
        
        // Initialize ingredient availability
        if (matchingRecipe.ingredients) {
          const initialAvailability: { [key: string]: boolean } = {};
          matchingRecipe.ingredients.forEach((ing, index) => {
            initialAvailability[`${index}-${ing.name}`] = false;
          });
    ...
    return (
      <div className="flex justify-center items-center h-screen">
        <div className="animate-bounce flex flex-col items-center">
          ...
          <p className="mt-4 text-lg">Preparing your recipe...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="card bg-base-100 shadow-xl p-8 text-center">
          <h2 className="text-2xl font-bold text-error">Recipe Not Found</h2>
          <p className="mt-4">{error || 'The recipe you are looking for could not be found.'}</p>
          <button 
  ...
  const scalingFactor = currentPortions / (recipe.servings || recipe.portion || 1);
  ...
  const scaledNutritionalInfo = recipe.nutritional_info
    ? {
        ...
        carbohydrates: +(recipe.nutritional_info.carbohydrates * scalingFactor).toFixed(1),
        ...
        cholesterol: +(recipe.nutritional_info.cholesterol * scalingFactor).toFixed(1),
      }
    : null;
          ...
          <div className="mb-6 sm:mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 sm:gap-3">
              {recipe.ingredients?.map((ingredient, index) => {
                const scaledQuantity = (ingredient.quantity * scalingFactor).toFixed(
                  ingredient.quantity % 1 === 0 ? 0 : 1
                );
                const ingredientKey = `${index}-${ingredient.name}`;
                
                return (
                  <div key={ingredientKey} className="flex items-center gap-2 p-2 rounded-lg hover:bg-base-200 transition-colors">
                    <input
                      type="checkbox"
                    ...
                    />
                    <span className={availableIngredients[ingredientKey] ? "line-through opacity-60 text-sm sm:text-base" : "text-sm sm:text-base"}>
                      <span className="font-medium">{scaledQuantity} {ingredient.unit}</span> {ingredient.name}
                    </span>
                  </div>
                );
              })}
          ...
          {steps.length > 0 && (
            <div className="mb-6 sm:mb-8">
              ...
              <div className="space-y-4 sm:space-y-6">
                {steps.map((step) => (
                  <div key={step.order} className="p-3 sm:p-4 bg-base-200 rounded-xl">
            ...
            <>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-4 bg-base-200 rounded-xl">
                <div className="stat bg-base-100 rounded-lg shadow-sm">
                  ...
                  <div className="stat-value text-primary text-2xl">{scaledNutritionalInfo.calories}</div>
                </div>
                <div className="stat bg-base-100 rounded-lg shadow-sm">
                  <div className="stat-title text-xs">Protein</div>
                  <div className="stat-value text-2xl">{scaledNutritionalInfo.protein}g</div>
                </div>
                <div className="stat bg-base-100 rounded-lg shadow-sm">
                  <div className="stat-title text-xs">Fat</div>
                  <div className="stat-value text-2xl">{scaledNutritionalInfo.fat}g</div>
                </div>
                <div className="stat bg-base-100 rounded-lg shadow-sm">
                  <div className="stat-title text-xs">Carbs</div>
                  <div className="stat-value text-2xl">{scaledNutritionalInfo.carbohydrates}g</div>
                </div>
                <div className="stat bg-base-100 rounded-lg shadow-sm">
                  <div className="stat-title text-xs">Fiber</div>
                  <div className="stat-value text-2xl">{scaledNutritionalInfo.fiber}g</div>
                </div>
                <div className="stat bg-base-100 rounded-lg shadow-sm">
                  <div className="stat-title text-xs">Sugar</div>
                  <div className="stat-value text-2xl">{scaledNutritionalInfo.sugar}g</div>
                </div>
                <div className="stat bg-base-100 rounded-lg shadow-sm">
                  <div className="stat-title text-xs">Sodium</div>
                  <div className="stat-value text-2xl">{scaledNutritionalInfo.sodium}mg</div>
                </div>
                <div className="stat bg-base-100 rounded-lg shadow-sm">
                  <div className="stat-title text-xs">Cholesterol</div>
                  <div className="stat-value text-2xl">{scaledNutritionalInfo.cholesterol}mg</div>
                </div>
              </div>
            </>
          )}
