...
import { IconArrowDown, IconX, IconClipboardList, IconChecklist, IconHelpCircle, IconBulb, IconStars, IconRocket, IconShoppingCart, IconListCheck } from '@tabler/icons-react';
                ...
                return (
                  <div key={ingredient.ingredient_id} className="flex items-center gap-2 p-2 rounded-lg hover:bg-base-200 transition-colors">
                    ...
                    <span className={availableIngredients[ingredient.ingredient_id] ? "line-through opacity-60 text-sm sm:text-base" : "text-sm sm:text-base"}>
                      <span className="font-medium">{scaledQuantity} {ingredient.unit}</span> {ingredient.ingredient.name}
                    </span>
                  </div>
                );
              })}
